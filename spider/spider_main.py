...
class WriterProcess(Process):
    ...
    def run(self):
        ...
        id = 1
        with open('data/qcwy.csv', 'a', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            while True:
                if id == self.number + 1:
                    f.close()
...
def main(dict_parameter):
    ...
    if os.path.exists('./data/qcwy.csv'):
        os.remove('./data/qcwy.csv')
    ...
    for city in citys:
        for job in jobs:
            ...
            if no * number >= total:
                ...
                os.system('csvtotable ./data/qcwy.csv ./templates/data.html')
                p2.terminate()
                return
            p1.join()
            no = no + 1
